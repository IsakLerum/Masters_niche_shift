...
# https://doi.org/10.1002/ece3.6036 
...
#  Gbif
#  Reference code: GBIF.org (23 March 2023) GBIF Occurrence Download https://doi.org/10.15468/dl.4yurdw
#
...
climfull <- c(bio1, bio6, bio12, bio14)
...
model_range_native <- c(-9, -3.4, 35.8, 41)

# clean occurrence data
Rhododendron_native <- Rhododendron_occurence_clean |>
  filter(
...
# The function is available in the Isak_functions.R file
occurence_grid_native <- make_occurence_grid(Rhododendron_native, climate_native)


## 1.8 Invasive population ==============

# This defines the geographical range of the invasive model (W, E, S, N)
model_range_invasive <- c(-7.6, 1.9, 49.9, 59.4)

# clean occurrence data
Rhododendron_invasive <- Rhododendron_occurence_clean |>
  filter(
...
climate_exclusion_range1 <- c(-7.6, -5.3, 51.7, 55.26)

climate_exclusion1 <- define_range_and_aggregation(climfull, climate_exclusion_range1)

climate_exclusion_null1 <- setValues(climate_exclusion1, values = NA)

climate_invasive_merge1 <- terra::merge(climate_exclusion_null1, climate_invasive)

climate_exclusion_range2 <- c(1, 1.9, 49.9, 51)

climate_exclusion2 <- define_range_and_aggregation(climfull, climate_exclusion_range2)

climate_exclusion_null2 <- setValues(climate_exclusion2, values = NA)

climate_invasive_merge2 <- terra::merge(climate_exclusion_null2, climate_invasive_merge1)

climate_exclusion_range3 <- c(-7.6, -6.6, 55, 55.5)

climate_exclusion3 <- define_range_and_aggregation(climfull, climate_exclusion_range3)

climate_exclusion_null3 <- setValues(climate_exclusion3, values = NA)

climate_invasive_final <- terra::merge(climate_exclusion_null3, climate_invasive_merge2) |> 
  raster::aggregate(2)

# writeRaster(climate_invasive_final, "Manzoor_climate_invasive_agg2.tif")

climate_invasive_final <- rast("Manzoor_climate_invasive_agg3.tif")
...
# The function is available in the Isak_functions.R file
occurence_grid_invasive <- make_occurence_grid(Rhododendron_invasive, climate_invasive_final)


# ## 1.9 Plot occurrences ==============
 
native_coastline <- ne_countries(continent = c("europe"), returnclass = "sf", scale = 50) 

# (-9, -3.4, 35.8, 41) model_range_native[2]

occ_nat <- ggplot() +
  geom_sf(data = native_coastline, fill = "white") +
  geom_point(data = Rhododendron_native, aes(Longitude, Latitude), size = 1) +
...
occ_inv <- ggplot() +
  ...
  geom_point(data = Rhododendron_invasive, aes(Longitude, Latitude), size = 0.05) +
  scale_x_continuous(expand = c(0, 0),
...
mod_native <- sdmINLASpatRandEff(occurence_grid_native, climate_grid_native,
                                 ...
                                 myName = "Rhododendron_Native_agg"
)
#agg = 2
...
mod_invasive <- sdmINLASpatRandEff(occurence_grid_invasive, climate_grid_invasive,
                                   ...
                                   createGeoTIFF = FALSE,
                                   outFolder = getwd(),
                                   myName = "Rhododendron_Invasive_agg"
)
#agg = 3
...
obj_native <- readRDS(file = "Rhododendron_Native_agg.rds")
...
Rhododendron_AUC_native <- auc(roc_object_native)
Rhododendron_AUC_native


## 3.2 Invasive Results =========
# Retrive model results
obj_invasive <- readRDS(file = "Rhododendron_invasive_agg.rds")
...
Rhododendron_AUC_invasive <- auc(roc_object_invasive)
Rhododendron_AUC_invasive
...
response_curve_native <- with(integrated_curves$wc2.1_30s_bio_14$native,
                              respCurve(xSeq, datSel, pars, clim_codes[4], integral))
response_curve_invasive <- with(integrated_curves$wc2.1_30s_bio_14$invasive,
                                respCurve(xSeq, datSel, pars, clim_codes[4], integral))

with(integrated_curves$wc2.1_30s_bio_14$invasive,
     plot(xSeq, response_curve_invasive,
          xlim = range(c(xSeq, integrated_curves$wc2.1_30s_bio_14$native$xSeq)), col = "red", 
          ylim = c(0, max(c(response_curve_native, response_curve_invasive))),
          type = "l", xlab = "",
          ylab = "",
          axes = FALSE))

with(integrated_curves$wc2.1_30s_bio_14$native, lines(xSeq, response_curve_native, col = "blue"))
abline(v = integrated_curves$wc2.1_30s_bio_14$cRange, lwd = 2, lty = 2)
legend("topright", paste0("Overlap = ", round(all_overlaps[4], digits = 3)), bty = "o")
box()
axis(2, las = 1)
...
vioplot(wc2.1_30s_bio_14~occurrence, data = climate_at_occurrences_native,
        side = "right", horizontal = TRUE, col = "blue",
        ylim = range(c(integrated_curves$wc2.1_30s_bio_14$invasive$xSeq,
                       integrated_curves$wc2.1_30s_bio_14$native$xSeq)),
        ylab = "", xlab = clim_names[4], xaxt = "n")

abline(h = 1, lwd = 2, lty = 2)

vioplot(wc2.1_30s_bio_14~occurrence, data = climate_at_occurrences_invasive,
...
# Single curve
niche_overlap_plot(integrated_curves$wc2.1_30s_bio_14, clim_codes[4], clim_names[4], climate_at_occurrences_native, climate_at_occurrences_invasive, "test", all_overlaps[4])


# All curves
Map(niche_overlap_plot, integrated_curves, clim_codes, clim_names, list(climate_at_occurrences_native), list(climate_at_occurrences_invasive), "Manzoor", all_overlaps)
