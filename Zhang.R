# Zhang 2022 -------------------------------
# https://doi.org/10.1093/jpe/rtab086
...
# Supplementary table 2, from the original paper
#
# Source of climate data:
# WorldClim, 2.5 arc-minutes, all standard (19) WorldClim variables + elevation,
# https://www.worldclim.org/data/worldclim21.html
#
# Notes about the dataset:
# Native data has been somewhat expanded to cover occurrences bordering China (to accommodate rectangular raster)
...
Galinsoga_occurence <- read_csv(file = paste0(here(), "/data/Galinsoga_quadriradiata.csv"))

# Clean the data if necessary
# Should include numeric colums for Latitude, Longitude and occurrenceStatus (which should always be 1)
Galinsoga_occurence_clean <- Galinsoga_occurence |>
...
bio18 <- rast("data/wc2.1_2.5m_bio/wc2.1_2.5m_bio_18.tif")
bio_elev <- rast("data/wc2.1_2.5m_elev/wc2.1_2.5m_elev.tif")



# Add all climate variable into a single terra object
climfull <- c(bio5, bio6, bio18, bio_elev)

# Make a list of names for each climate variable (make sure to have the correct order)
clim_names <- c("Max Temperature of Warmest Month (\u00B0C)",
...
model_range_native <- c(-112.4, -37.8, -34.1, 31.3)

# clean occurrence data
Galinsoga_native <- Galinsoga_occurence_clean |>
  filter(
...
exclusion_range <- c(-87, -79.5, 25, 33)

Galinsoga_exclusion <- Galinsoga_occurence_clean |>
  filter(
    Longitude > exclusion_range[1], Longitude < exclusion_range[2],
    Latitude > exclusion_range[3], Latitude < exclusion_range[4]
  )

Galinsoga_native_new <- Galinsoga_native |> 
  anti_join(Galinsoga_exclusion)
...
# The function is available in the Isak_functions.R file
occurence_grid_native <- make_occurence_grid(Galinsoga_native_new, climate_native_new)
...
model_range_invasive <- c(79, 129.6, 22.3, 47.1)

# clean occurrence data
Galinsoga_invasive <- Galinsoga_occurence_clean |>
  filter(
...
# The function is available in the Isak_functions.R file
occurence_grid_invasive <- make_occurence_grid(Galinsoga_invasive, climate_invasive)


# ## 1.9 Plot occurrences ==============

native_coastline <- ne_countries(continent = c("south america", "north america"), returnclass = "sf", scale = 50) 

occ_nat <- ggplot() +
  geom_sf(data = native_coastline, fill = "white") +
  geom_point(data = Galinsoga_native_new, aes(Longitude, Latitude), size = 1) +
...
invasive_coastline <- ne_countries(continent = "asia", returnclass = "sf", scale = 50)

occ_inv <- ggplot() +
  geom_sf(data = invasive_coastline, fill = "white") +
  geom_point(data = Galinsoga_invasive, aes(Longitude, Latitude)) +
  scale_x_continuous(expand = c(0, 0),
...
mod_native <- sdmINLASpatRandEff(occurence_grid_native, climate_grid_native,
                                 ...
                                 myName = "Galinsoga_Native_agg2"
)
# agg2 made with factor = 3
...
obj_native <- readRDS(file = "Galinsoga_Native_agg2.rds")
...
Galinsoga_AUC_native <- auc(roc_object_native)
Galinsoga_AUC_native


## 3.2 Invasive Results =========
# Retrive model results
obj_invasive <- readRDS(file = "Galinsoga_invasive_agg.rds")
...
Galinsoga_AUC_invasive <- auc(roc_object_invasive)
Galinsoga_AUC_invasive
...
# All curves
Map(niche_overlap_plot, integrated_curves, clim_codes, clim_names, list(climate_at_occurrences_native), list(climate_at_occurrences_invasive), "Zhang", all_overlaps)
