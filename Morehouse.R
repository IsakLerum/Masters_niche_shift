# Morehouse 2013 -------------------------------
# https://doi.org/10.1163/1937240X-00002120
...
#  Gbif
#  Reference code: GBIF.org (23 March 2023) GBIF Occurrence Download https://doi.org/10.15468/dl.h8y5zy
#
# If the original data is not available, state how the data set was recreated

# U.S. Geological Survey, 2023, Specimen observation data for <i>Faxonius rusticus</i> (Girard, 1852), Nonindigenous Aquatic Species Database, Gainesville, FL, 
# https://nas.er.usgs.gov/queries/CollectionInfo.aspx?SpeciesID=214, Access Date: 3/24/2023.

# https://biocoll.inhs.illinois.edu/portal/collections/misc/collprofiles.php?collid=52&stat=geography&country=United%20States&state=#geographystats 


# Genus was changed to Faxonius in 2017 and seems to have united several species, expanding the occurences

# requires custom border on native range
...
# Author decided to rescale climate grid to 1 km^2, we will not do so
...
rusticus_occurence <- read_csv(file = paste0(here(), "/data/NAS-Data-Download-Orconectes-Rusticus/NAS-Data-Download-Orconectes-Rusticus.csv"))

inhs_crustacean <- read_csv(file = paste0(here(), "/data/INHS-CRUSTACEAN_DwC-A/occurrences.csv")) 

rusticus_occurence_2 <- inhs_crustacean


# Clean the data if necessary
# Should include numeric colums for Latitude, Longitude and occurrenceStatus (which should always be 1)
rusticus_occurence_clean <- rusticus_occurence |>
  mutate(occurrenceStatus = 1) |>
  filter(Year < 2013) |> 
  select(Latitude, Longitude, occurrenceStatus) 
  
rusticus_occurence_clean_2 <- rusticus_occurence_2 |> 
  filter(scientificName == "Faxonius rusticus", year < 2013) |> 
  mutate(Latitude = as.double(decimalLatitude),
         Longitude = as.double(decimalLongitude),
         occurrenceStatus = 1) |>
  select(Latitude, Longitude, occurrenceStatus)

rusticus_occurence_final <- full_join(rusticus_occurence_clean, rusticus_occurence_clean_2)

## 1.5 Import climate data ========
# This example data comes from Worldclim
bio3 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_3.tif")
bio5 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_5.tif")
bio7 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_7.tif")
bio9 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_9.tif")
bio10 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_10.tif")
bio15 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_15.tif")
bio16 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_16.tif")
bio17 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_17.tif")
bio18 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_18.tif")
bio19 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_19.tif")


# Add all climate variable into a single terra object
climfull <- c(bio3, bio5, bio7, bio9, bio10, bio15, bio16, bio17, bio18, bio19)

# Make a list of names for each climate variable (make sure to have the correct order)
clim_names <- c("Isothermality (BIO2/BIO7) (*100)", 
...
model_range_native <- c(-87.3, -81.1, 36.6, 41.9)

# clean occurrence data
rusticus_native <- rusticus_occurence_final |>
  filter(
...
exclusion_range <- c(-82.6, -81.7, 38, 38.9)

rusticus_native_exclusion <- rusticus_occurence_final |>
  filter(
    Longitude > exclusion_range[1], Longitude < exclusion_range[2],
    Latitude > exclusion_range[3], Latitude < exclusion_range[4]
  )

rusticus_native_final <- anti_join(rusticus_native, rusticus_native_exclusion)


# Prepare data
climate_native <- define_range_and_aggregation(climfull, model_range_native)

climate_native_exclusion <- define_range_and_aggregation(climfull, exclusion_range, aggregation_factor = 1)

climate_native_exclusion_null <- setValues(climate_native_exclusion, values = NA)

climate_native_new <- terra::merge(climate_native_exclusion_null, climate_native) |> 
  raster::aggregate(3)
# base 3
# X 2
...
# The function is available in the Isak_functions.R file
occurence_grid_native <- make_occurence_grid(rusticus_native_final, climate_native_new)


## 1.8 Invasive population ==============

# This defines the geographical range of the invasive model (W, E, S, N)
model_range_invasive <- c(-122.5, -66.7, 33, 50.9)

# clean occurrence data and remove occurences belonging to the native area
rusticus_invasive <- rusticus_occurence_final |>
  filter(
  ...
  anti_join(rusticus_native_final)


# Prepare data
climate_invasive <- define_range_and_aggregation(climfull, model_range_invasive)

climate_invasive_exclusion <- setValues(climate_native, values = NA)

climate_invasive_merge <- terra::merge(climate_invasive_exclusion, climate_invasive)

climate_invasive_inclusion_range <- c(-82.6, -80, 36, 38.9)

climate_invasive_inclusion <- define_range_and_aggregation(climfull, climate_invasive_inclusion_range)

climate_invasive_final <- terra::merge(climate_invasive_inclusion, climate_invasive_merge) |> 
  raster::aggregate(13)
# base 13
# X try 9


# climate_invasive_new <- climate_invasive_new |> 
#   raster::aggregate(7)

# The function is available in the Isak_functions.R file
climate_grid_invasive <- make_climate_grid(climate_invasive_final)


# The function is available in the Isak_functions.R file
occurence_grid_invasive <- make_occurence_grid(rusticus_invasive, climate_invasive_final)
...
ggplot() +
  ...
  geom_point(data = rusticus_native, aes(Longitude, Latitude, color = "native"), size = 1) +
  geom_point(data = rusticus_invasive, aes(Longitude, Latitude, color = "invasive"), size = 1) +
...
mod_invasive <- sdmINLASpatRandEff(occurence_grid_invasive, climate_grid_invasive,
                                   ...
                                   createGeoTIFF = FALSE,
                                   outFolder = getwd(),
                                   myName = "rusticus_Invasive_agg_X"
)
#agg = 13
#X = x
...
obj_native <- readRDS(file = "rusticus_Native_agg.rds")
...
rusticus_AUC_native <- auc(roc_object_native)
rusticus_AUC_native


## 3.2 Invasive Results =========
# Retrive model results
obj_invasive <- readRDS(file = "rusticus_invasive_agg.rds")
...
rusticus_AUC_invasive <- auc(roc_object_invasive)
rusticus_AUC_invasive
...
response_curve_native <- with(integrated_curves$wc2.1_30s_bio_19$native,
                              respCurve(xSeq, datSel, pars, clim_codes[10], integral))
response_curve_invasive <- with(integrated_curves$wc2.1_30s_bio_19$invasive,
                                respCurve(xSeq, datSel, pars, clim_codes[10], integral))

with(integrated_curves$wc2.1_30s_bio_19$invasive,
     plot(xSeq, response_curve_invasive,
          xlim = range(c(xSeq, integrated_curves$wc2.1_30s_bio_19$native$xSeq)), col = "red", 
          ylim = c(0, max(c(response_curve_native, response_curve_invasive))),
          type = "l", xlab = "",
          ylab = "",
          axes = FALSE))

with(integrated_curves$wc2.1_30s_bio_19$native, lines(xSeq, response_curve_native, col = "blue"))
abline(v = integrated_curves$wc2.1_30s_bio_19$cRange, lwd = 2, lty = 2)
legend("topright", paste0("Overlap = ", round(all_overlaps[10], digits = 3)), bty = "o")
box()
axis(2, las = 1)
...
vioplot(wc2.1_30s_bio_19~occurrence, data = climate_at_occurrences_native,
        side = "right", horizontal = TRUE, col = "blue",
        ylim = range(c(integrated_curves$wc2.1_30s_bio_19$invasive$xSeq,
                       integrated_curves$wc2.1_30s_bio_19$native$xSeq)),
        ylab = "", xlab = clim_names[10], xaxt = "n")

abline(h = 1, lwd = 2, lty = 2)

vioplot(wc2.1_30s_bio_19~occurrence, data = climate_at_occurrences_invasive,
...
# Single curve
niche_overlap_plot(integrated_curves$wc2.1_30s_bio_3, clim_codes[1], clim_names[1], climate_at_occurrences_native, climate_at_occurrences_invasive, "test", all_overlaps[1])


# All curves
Map(niche_overlap_plot, integrated_curves, clim_codes, clim_names, list(climate_at_occurrences_native), list(climate_at_occurrences_invasive), "Morehouse", all_overlaps)
...
# spat_effect_nat <- (stats::var(occ - clim_pred) - stats::var(occ - meanEst)) / stats::var(occ)

native_realised_estimate <- obj_native$spatialPredictions$meanEst
summary(native_realised_estimate)

native_fundamental_estimate <- invLogit(obj_native$spatialPredictions$meanClimatePred)
summary(native_fundamental_estimate)

native_occurences <- replace(occurence_grid_native$occurrence, is.na(native_fundamental_estimate), NA)
summary(native_occurences)

residuals_native <- native_occurences - native_realised_estimate
summary(residuals_native)


explained_variance_native <- 1 - (stats::var(residuals_native, na.rm = TRUE) /
                                    stats::var(native_occurences, na.rm = TRUE))
explained_variance_native


spatial_effect_native <- (stats::var(native_occurences - native_fundamental_estimate, na.rm = TRUE) -
                            stats::var(native_occurences - native_realised_estimate, na.rm = TRUE)) /
  stats::var(native_occurences, na.rm = TRUE)

spatial_effect_native

spatial_proportion_native <- spatial_effect_native/explained_variance_native
spatial_proportion_native

climate_effect_native <- explained_variance_native - spatial_effect_native
climate_effect_native


invasive_realised_estimate <- obj_invasive$spatialPredictions$meanEst
...
residuals_invasive <- invasive_occurences - invasive_realised_estimate
summary(residuals_invasive)


explained_variance_invasive <- 1 - (stats::var(residuals_invasive, na.rm = TRUE) /
                                      stats::var(invasive_occurences, na.rm = TRUE))
explained_variance_invasive


spatial_effect_invasive <- (stats::var(invasive_occurences - invasive_fundamental_estimate, na.rm = TRUE) -
                              stats::var(invasive_occurences - invasive_realised_estimate, na.rm = TRUE)) /
  stats::var(invasive_occurences, na.rm = TRUE)

spatial_effect_invasive

spatial_proportion_invasive <- spatial_effect_invasive/explained_variance_invasive
spatial_proportion_invasive

climate_effect_invasive <- explained_variance_invasive - spatial_effect_invasive
climate_effect_invasive
