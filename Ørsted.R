# Ørsted 2019 -------------------------------
# https://dx.doi.org/10.1111/1365-2664.13285
#
# 1 Preparation ---------------------
#
## 1.1 Data sources ==================
...
#  Data-dryad:  https://doi.org/10.5061/dryad.mn0254p
#
# Source of climate data:
# WorldClim, 30 arc-seconds, all standard (19) WorldClim variables,
...
drosophila_occurence <- read_xlsx(path = "data/JPE-Oersted-ST1.xlsx", sheet = 1, skip = 1)

# Clean the data if necessary
# Should include numeric colums for Latitude, Longitude and occurrenceStatus (which should always be 1)
drosophila_occurence_clean <- drosophila_occurence |>
  mutate(Latitude = as.double(Latitude), Longitude = as.double(Longitude)) |>
  select(Latitude, Longitude, occurrenceStatus) |>
  replace(is.character(drosophila_occurence$occurrenceStatus) == TRUE, drosophila_occurence$occurrenceStatus <- 1)

## 1.5 Import climate data ========
# This example data comes from Worldclim
bio01 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_1.tif")
bio02 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_2.tif")
bio04 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_4.tif")
bio05 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_5.tif")
bio06 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_6.tif")
bio07 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_7.tif")
bio11 <- rast("data/wc2.1_30s_bio/wc2.1_30s_bio_11.tif")
...
climfull <- c(bio01, bio02, bio04, bio05, bio06, bio07, bio11, bio12, bio17)
...
# #NOTE: ERROR with invasive range!!! --------------
# model_range_extended <- c(5, 15, 42, 54)
# 
...
model_range_native <- c(66.4, 148.8, 8.5, 49.8)
# S = 10.5
# clean occurrence data
drosophila_native <- drosophila_occurence_clean |>
  filter(
...
# Prepare data
climate_native <- define_range_and_aggregation(climfull, model_range_native, aggregation_factor = 17)
# base 17
# X 
...
# The function is available in the Isak_functions.R file
occurence_grid_native <- make_occurence_grid(drosophila_native, climate_native)


## 1.8 Invasive population ==============

# This defines the geographical range of the invasive model (W, E, S, N)
model_range_invasive <- c(-17.4, 44.2, 31.3, 60.9)
# arbitrary -17.4 to reduce grid

# clean occurrence data
drosophila_invasive <- drosophila_occurence_clean |>
  filter(
...
# summary(drosophila_invasive)
# (max(drosophila_invasive$Latitude) - min(drosophila_invasive$Latitude)) * 0.05


# Prepare data
# climate_invasive <- define_range_and_aggregation(climfull, model_range_invasive, aggregation_factor = 13)

# writeRaster(climate_invasive, "Orsted_invasive_agg13.tif")

climate_invasive <- rast("Orsted_invasive_agg13.tif")
# plot(climate_invasive$wc2.1_30s_bio_1)

# The function is available in the Isak_functions.R file
climate_grid_invasive <- make_climate_grid(climate_invasive)

# The function is available in the Isak_functions.R file
occurence_grid_invasive <- make_occurence_grid(drosophila_invasive, climate_invasive)



# # Alt excluding the atlantic islands
# model_range_invasive_alt <- c(-11.1, 43.8, 31.3, 60.9)
# # arbitrary -17.4 to reduce grid
# 
# # clean occurrence data
# drosophila_invasive_alt <- drosophila_occurence_clean |>
#   filter(
#     Longitude > model_range_invasive_alt[1], Longitude < model_range_invasive_alt[2],
#     Latitude > model_range_invasive_alt[3], Latitude < model_range_invasive_alt[4]
#   )
# 
# # summary(drosophila_invasive_alt)
# # (max(drosophila_invasive_alt$Longitude) - min(drosophila_invasive_alt$Longitude)) * 0.05
# 
# 
# # Prepare data
# climate_invasive_alt <- define_range_and_aggregation(climfull, model_range_invasive_alt,
#                                                      aggregation_factor = 12)
# 
# # writeRaster(climate_invasive, "Orsted_invasive_alt_agg12.tif")
# 
# # climate_invasive_alt <- rast("Orsted_invasive_alt_agg13.tif")
# 
# # plot(climate_invasive_alt$wc2.1_30s_bio_1)
# 
# # The function is available in the Isak_functions.R file
# climate_grid_invasive_alt <- make_climate_grid(climate_invasive_alt)
# 
# # The function is available in the Isak_functions.R file
# occurence_grid_invasive_alt <- make_occurence_grid(drosophila_invasive_alt, climate_invasive_alt)
...
# 
native_coastline <- ne_countries(continent = c("asia"), returnclass = "sf", scale = 50) 

occ_nat <- ggplot() +
  geom_sf(data = native_coastline, fill = "white") +
  geom_point(data = drosophila_native, aes(Longitude, Latitude), size = 1) +
...
invasive_coastline <- ne_countries(continent = c("europe", "africa", "asia"), returnclass = "sf", scale = 50)

occ_inv <- ggplot() +
  geom_sf(data = invasive_coastline, fill = "white") +
  geom_point(data = drosophila_invasive, aes(Longitude, Latitude), size = 0.9) +
  scale_x_continuous(expand = c(0, 0),
...
mod_invasive <- sdmINLASpatRandEff(occurence_grid_invasive, climate_grid_invasive,
                                   ...
                                   createGeoTIFF = FALSE,
                                   outFolder = getwd(),
                                   myName = "drosophila_Invasive_agg"
)
#agg = 13


mod_invasive_alt <- sdmINLASpatRandEff(occurence_grid_invasive_alt, climate_grid_invasive_alt,
                                   meshParameters = list(max.n.strict = -1),
                                   createGeoTIFF = FALSE,
                                   outFolder = getwd(),
                                   myName = "drosophila_Invasive_alt"
)
...
obj_native <- readRDS(file = "drosophila_Native_agg.rds")
...
x_nat <- ggplot(native_occurence_estimate) +
  ...
  scale_x_continuous(expand = c(0, 0), limits = c(model_range_native[1], model_range_native[2])) +
  scale_y_continuous(expand = c(0, 0), limits = c(model_range_native[3], model_range_native[4])) +
  labs(title = "a) Native realised niche", x = NULL, y = NULL) +
...
y_nat <- ggplot(native_suitability) +
  ...
  scale_x_continuous(expand = c(0, 0), limits = c(model_range_native[1], model_range_native[2])) +
  scale_y_continuous(expand = c(0, 0), limits = c(model_range_native[3], model_range_native[4])) +
  labs(title = "b) Native fundamental niche", x = NULL, y = NULL) +
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"), panel.background = element_rect(fill = "lightblue"))
y_nat
...
drosophila_AUC_native <- auc(roc_object_native)
drosophila_AUC_native


## 3.2 Invasive Results =========
# Retrive model results
obj_invasive <- readRDS(file = "drosophila_invasive_agg.rds")
...
drosophila_AUC_invasive <- auc(roc_object_invasive)
drosophila_AUC_invasive
...
response_curve_native <- with(integrated_curves$wc2.1_30s_bio_17$native,
                              respCurve(xSeq, datSel, pars, clim_codes[9], integral))
response_curve_invasive <- with(integrated_curves$wc2.1_30s_bio_17$invasive,
                                respCurve(xSeq, datSel, pars, clim_codes[9], integral))

with(integrated_curves$wc2.1_30s_bio_17$invasive,
     plot(xSeq, response_curve_invasive,
          xlim = range(c(xSeq, integrated_curves$wc2.1_30s_bio_17$native$xSeq)), col = "red", 
          ylim = c(0, max(c(response_curve_native, response_curve_invasive))),
          type = "l", xlab = "",
          ylab = "",
          axes = FALSE))

with(integrated_curves$wc2.1_30s_bio_17$native, lines(xSeq, response_curve_native, col = "blue"))
abline(v = integrated_curves$wc2.1_30s_bio_17$cRange, lwd = 2, lty = 2)
legend("topright", paste0("Overlap = ", round(all_overlaps[9], digits = 3)), bty = "o")
box()
axis(2, las = 1)
...
vioplot(wc2.1_30s_bio_17~occurrence, data = climate_at_occurrences_native,
        side = "right", horizontal = TRUE, col = "blue",
        ylim = range(c(integrated_curves$wc2.1_30s_bio_17$invasive$xSeq,
                       integrated_curves$wc2.1_30s_bio_17$native$xSeq)),
        ylab = "", xlab = clim_names[9], xaxt = "n")

abline(h = 1, lwd = 2, lty = 2)

vioplot(wc2.1_30s_bio_17~occurrence, data = climate_at_occurrences_invasive,
...
# Single curve
niche_overlap_plot(integrated_curves$wc2.1_30s_bio_17, clim_codes[9], clim_names[9], climate_at_occurrences_native, climate_at_occurrences_invasive, "test", all_overlaps[1])


# All curves
Map(niche_overlap_plot, integrated_curves, clim_codes, clim_names, list(climate_at_occurrences_native), list(climate_at_occurrences_invasive), "Orsted", all_overlaps)
